...
# ╔═╡ e9493b2c-fc9b-11ee-24e7-c365c43c78ac
begin
	using UnROOT, DataFrames, StatsPlots, FHist, StatsBase, Distributions, Turing, StatsBase
end

# ╔═╡ 63592621-1d64-4c66-bdcc-369cbcc71b3c
using LinearAlgebra

# ╔═╡ abe2d0f0-0cbf-4434-8324-edee9a72949a
md"""
## Load necessary packages
"""

# ╔═╡ 9ed11e88-0146-448c-9c75-61fe675afbef
theme(:dao)

# ╔═╡ b78f421d-7b6c-4cec-9c18-bda54d595452
# load root file
f2nu = ROOTFile("../data/CAT/bb2nu/bb2nu_1M_events.root")

# ╔═╡ 461f38c0-1351-4666-b143-eb1c10f42a5b
keys(f2nu["tree"])

# ╔═╡ d14a1b19-0fc7-4042-8b86-a1c728ed356e
# extract reconstructed vertex positions into dataframe
df = LazyTree(f2nu, "tree", ["x1Escaped","y1Escaped","z1Escaped","x2Escaped","y2Escaped","z2Escaped"]) |> DataFrame

# ╔═╡ a3dc4a17-ae91-4f34-a3cf-00cfd73502b8
# quick glance at what's inside
describe(df)

# ╔═╡ 74053860-5317-4f40-be4b-f2228196be82
md"""
# Analysis:
...
### For each histogram we fit and extract FWHM:
"""

# ╔═╡ 94cd7ef8-69f5-450e-8f03-404e6cb25ebf
...
# ╔═╡ c0287649-461c-42f6-96c1-8f96698847ce
describe(df)

# ╔═╡ c0173ba8-6d12-4ef2-ab14-3a67b76b5fbe
begin
	h1dy = StatsBase.fit(Histogram{Float32}, df.dy, -235:5:235)
	h1dz = StatsBase.fit(Histogram{Float32}, df.dz, -235:5:235)
end

# ╔═╡ 7f700bf6-37a1-417f-b8b1-f80080c0d695
rms(v::Vector{<:Real}) = std(v; corrected=:false, mean=0)

# ╔═╡ a529cf91-b5de-41c9-84a1-6d7fb2ceb752
let 
...
# ╔═╡ b94cc5e9-9133-4d0c-b9af-536a316ec5df
md"""
# We can try to fit the data to see if we can get the FWHM: 

"""

# ╔═╡ ae302643-214a-47af-87a2-78f6370a6059
...
# ╔═╡ 01ac2bde-8d45-473c-b32d-5eb7f046c502
mdy = model_dy(df.dy)

# ╔═╡ 7e6ff9da-b1df-4980-98f1-b5a2ab4746e5
chain_dy = Turing.sample(mdy, NUTS(), MCMCThreads(), 10000, 4)

# ╔═╡ 297a21f7-5e4c-4fc3-8da2-cfb87ce23f33
plot(chain_dy)

# ╔═╡ 5d3467dc-81a4-448f-b7c7-f2e2d97fbab2
fit_dy(μ, σ, x ) = pdf(Cauchy(μ, σ), x)

# ╔═╡ 5ee0d7bb-0209-4297-b731-6763d604bf8a
fit_mu, fit_sigma = mean(chain_dy[:,1,:]), mean(chain_dy[:,2,:])
...
manifest_format = "2.0"
project_hash = "a05825ed172351c6842e65bb2a2ca6de0c5dbda6"
...
git-tree-sha1 = "e82fd71badd88ee23d9ce748fb04f024fb952501"
...
git-tree-sha1 = "591cfa60ed9899839a312327cfe893c852b473c2"
...
git-tree-sha1 = "e9eeaf35b7e9a74511ded40fc688917072dbd897"
